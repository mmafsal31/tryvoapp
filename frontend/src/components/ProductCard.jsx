...
export default function FlipProductCardPremium({ product }) {
  ...
  // ⭐ NEW — Click timer to detect single vs double-click
  ...
  const handleClick = () => {
    if (clickTimeout) {
      ...
      clickTimeout = null;
      navigate(`/product/${product.id}`); // DOUBLE CLICK -> OPEN PRODUCT
    } else {
      clickTimeout = setTimeout(() => {
        setIsFlipped((prev) => !prev); // SINGLE CLICK -> FLIP
        clickTimeout = null;
  ...
  const storeId = product.store?.id || product.store_id || null;
  ...
  return (
    <div
      className="group perspective w-72 h-96 cursor-pointer mx-auto"
      onClick={handleClick}
    >
      <div
        ...
        <div className="absolute w-full h-full backface-hidden rounded-3xl overflow-hidden shadow-2xl bg-gray-900">
          <div className="relative w-full h-full">
            ...
            <div className="absolute top-4 left-4 flex flex-col gap-2">
              <span className="bg-black/80 text-white px-3 py-1 rounded-full font-bold text-sm shadow">
                ₹{price}
              </span>

              <span className="bg-black/80 text-yellow-400 px-3 py-1 rounded-full text-sm flex items-center gap-1 shadow">
                <Star className="w-4 h-4" /> {(product.rating || 4.5).toFixed(1)}
              </span>
            </div>
        ...
        <div className="absolute w-full h-full backface-hidden rotate-y-180 rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col justify-between h-full p-5">
            ...
            <div className="flex flex-col text-left gap-2">
              {storeId ? (
                <p
                  ...
                  className="text-[#DDF247] text-sm font-medium uppercase tracking-wider cursor-pointer hover:underline"
                >
                  {storeName}
                </p>
              ) : (
            ...
            <div className="flex flex-col items-center text-center gap-3">
              <h2 className="text-3xl font-extrabold text-white tracking-tight">₹{price}</h2>
              ...
              {availableSizes.length > 0 ? (
                <div className="flex flex-wrap justify-center gap-2 mt-1">
                  {availableSizes.map((s) => (
                    <span
                      key={s.size_label}
                      className="px-3 py-1 border border-gray-600 rounded-full text-sm font-medium text-gray-200"
                    >
                      {s.size_label}
                    </span>
                  ))}
            ...
            <div className="flex justify-center gap-3 mt-4">
              <button
                ...
                className="w-36 py-2 rounded-xl bg-[#DDF247] text-black font-semibold shadow-md"
              >
                View Product
              </button>
              ...
              {availableSizes.length > 0 ? (
                <Link
                  ...
                  className="w-36 py-2 rounded-xl text-center border border-[#DDF247] text-[#DDF247] font-semibold"
                >
                  Reserve
                </Link>
              ) : (
                <button
                  disabled
                  className="w-36 py-2 rounded-xl bg-gray-800 text-gray-500 font-semibold"
                >
                  Reserve Unavailable
                </button>
              )}
